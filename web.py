...
def initialize_plot(vertices, triangles, vertex_colors, instance_labels, id2labels):
    ...
    annotations = [
        ...
        if (category := label.split('_')[0]) not in excluded_categories
        if (instance_indices := np.where(instance_labels == int(instance_id))[0]).size > 0
        if (centroid := np.mean(vertices[instance_indices], axis=0)) is not None
    ]
...
with left_col:
    ...
    initialize_plot(vertices, triangles, vertex_colors, instance_labels, id2labels)
