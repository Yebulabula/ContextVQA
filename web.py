...
def initialize_plot(vertices, triangles, vertex_colors, instance_labels, id2labels):
    ...
    for instance_id, label in id2labels.items():
        ...
        if len(instance_indices) > 0:
            ...
            annotations.append(dict(
                ...
                y=centroid[1],
                z=centroid[2],
                text=f'<b>{category}</b>',  # Adding <b></b> to make the text bold
                showarrow=False,
                font=dict(size=12, color='yellow'),  # Slightly larger font size and yellow color
                xanchor='center',
            ))
    ...
    # Generate the summary text in a single line format
    summary_parts = []
    for category, labels in objects_by_category.items():
        object_count = len(labels)
        summary_parts.append(f"{object_count} {category + ('s' if object_count > 1 else '')}")

    summary_text = "This scene contains " + ", ".join(summary_parts) + "."

    # Display the summary markdown text
    st.markdown(f"\n{summary_text}")
    ...
    # Update layout with the updatemenu for toggling annotations
    fig.update_layout(
        scene=dict(
            aspectmode='data',
        ...
        updatemenus=[  # Use a dropdown to toggle annotations
            dict(
                type="dropdown",
                direction="down",
                buttons=[
...
with right_col:
    ...
    context_change = st.text_area("Describe the context change.", key="context_change", placeholder="Type here...", height=10)
