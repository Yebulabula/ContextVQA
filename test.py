...
final_data = open_json('/mnt/new_drive/Documents/ContextVQA/context_changes_human/final_total_change.json')

new_data = {}
count = 0
for key in final_data.keys():
    new_data[key] = []
    for description in final_data[key]:
        # check if the sentence start from the capital letter
        if description[0].isupper():
            new_data[key].append(description)
        else:
            count += 1
            
# save the new data
with open('final_total_change.json', 'w') as file:
